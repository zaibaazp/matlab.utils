function plotsignalintime_update(time2, xnoisy, startingsec, winsizesec, opts)
% plosignaltime.
%
%
...
subplot(7,1,[3,4])
plot(time2(thisInterval,:), xnoisy(thisInterval,1), time2(thisInterval,:),...
    xnoisy(thisInterval,2));
ylim([mi*0.8 ma*1.2]);
grid on;
axis tight;

subplot(7,1,[5,6])
plot(time2(thisInterval,:), xnoisy(thisInterval,3), time2(thisInterval,:),...
    xnoisy(thisInterval,4));
grid on;
axis tight;
